...
# had issues with theoretical orbital velocity so created a trial and error 
# system to find best one
def find_circular_orbit_velocity(separation, mass, box_size, grid_res, softening, 
                                  v_guess, n_test_steps=200, dt=0.001):
    """
    Find orbital velocity that gives circular orbit for given setup.
    Uses trial-and-error to find velocity that minimizes radial drift.
    """
...
# two equal masses, separation = 0.2
# for circular orbit: v = sqrt(G*M_total / (2*r)) for each particle
separation = 0.4
m1 = m2 = 1.0
M_total = m1 + m2
G = 1.0
...
# find velocity that gives circular orbit with our grid setup
...
sim2 = NBodySimulator(
    ...
    box_size=1.0,
    grid_res=128,  # higher resolution for binary
    dt=0.001,
    boundary='periodic',
    softening=0.2
)
...
axes[0].plot([pos2[0][0], pos2[1][0]], [pos2[0][1], pos2[1][1]], 
             'k--', alpha=0.3, label='Initial')
axes[0].legend()
...
plt.tight_layout()
plt.savefig('images/binary_orbit_test.png', dpi=150, bbox_inches='tight')
plt.show()
print("Saved plot to: binary_orbit_test.png in images folder")