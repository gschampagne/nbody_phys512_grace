...
pos_random = np.random.uniform(0, 1.0, size=(n_particles, 2)) # random positions 
vel_random = np.random.normal(0, 0.1, size=(n_particles, 2)) # random velocities 
mass_random = np.ones(n_particles) / n_particles # equal mass
...
axes[0, 1].scatter(sim_nonperiodic.particles.pos[in_box, 0], sim_nonperiodic.particles.pos[in_box, 1], s=0.1, alpha=0.5, c='blue')
...
periodic_compare = 100*(np.array(sim_periodic.energy_history) - sim_periodic.energy_history[0])/abs(sim_periodic.energy_history[0])
non_periodic_compare = 100*(np.array(sim_nonperiodic.energy_history) - sim_nonperiodic.energy_history[0])/abs(sim_nonperiodic.energy_history[0])
# energy comparison
axes[1, 0].plot(sim_periodic.time_history, periodic_compare ,'b-', linewidth=2, label='Periodic')
axes[1, 0].plot(sim_nonperiodic.time_history, non_periodic_compare ,'r-', linewidth=2, label='Non-Periodic')
axes[1, 0].set_xlabel('Time')
axes[1, 0].set_ylabel('Relative Energy Error (%)')
axes[1, 0].set_title('Energy Conservation Comparison')
axes[1, 0].legend()
# absolute energy
axes[1, 1].plot(sim_periodic.time_history, sim_periodic.energy_history,'b-', linewidth=2, label='Periodic')
axes[1, 1].plot(sim_nonperiodic.time_history, sim_nonperiodic.energy_history,'r-', linewidth=2, label='Non-Periodic')
